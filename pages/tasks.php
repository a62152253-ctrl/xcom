...
<div class="tasks-toolbar animate-slide-up">
    ...
    <div class="priority-filters">
        <button class="filter-tab active" data-filter="all" onclick="setPriorityFilter('all', this)"><i class="fa-solid fa-list"></i> Wszystkie</button>
        <button class="filter-tab" data-filter="Critical" onclick="setPriorityFilter('Critical', this)"><i class="fa-solid fa-fire"></i> Krytyczne</button>
        <button class="filter-tab" data-filter="High" onclick="setPriorityFilter('High', this)"><i class="fa-solid fa-arrow-up"></i> Wysokie</button>
        <button class="filter-tab" data-filter="Medium" onclick="setPriorityFilter('Medium', this)"><i class="fa-solid fa-minus"></i> Średnie</button>
        <button class="filter-tab" data-filter="Low" onclick="setPriorityFilter('Low', this)"><i class="fa-solid fa-arrow-down"></i> Niskie</button>
    </div>
...
<div class="kanban-board" id="kanban-board">
    ...
    <div class="kanban-column" id="col-<?= str_replace(' ', '-', strtolower($status)) ?>"
        ...
        <?php if (empty($cards)): ?>
        <div class="empty-state-premium" style="padding: 2rem 1rem; opacity: 0.7; margin: auto;">
            <div class="es-icon" style="width: 40px; height: 40px; margin: 0 auto 0.5rem;"><i class="fa-solid <?= $col_icons[$status] ?>" style="font-size: 1.5rem;"></i></div>
            <h4 class="es-title" style="font-size: 0.9rem; text-align: center; margin-bottom: 0.25rem;">Brak zadań</h4>
            <p class="es-sub" style="font-size: 0.75rem; text-align: center;">Przeciągnij tu zadanie lub dodaj nowe.</p>
        </div>
        <?php else: ?>
...
<input type="hidden" id="open-task-id-holder" value="<?= $open_task_id ?>">
